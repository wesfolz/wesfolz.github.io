...
const EventMarker = styled.div`
    ...
    left: ${(props) =>
      `calc(${
        props.imageSize * (props.multiplier || WIDTH_MULTIPLIER) +
        LINE_WIDTH / 2
      }px + 50vw)`};
    &::before {
        content: '';
        ...
        padding: ${(props) => `${props.height}vh`} ${(props) =>
  `${
    ...
    LINE_WIDTH / 4
        ...
        top: ${(props) =>
          `calc(${-props.height}vh - ${
            (INITIAL_SCALE * props.imageSize) / 4
          }px)`};
...
export default function Timeline(props) {
  ...
  const selectEvent = useCallback((ref, eventName) => {
    ...
    const yOffset =
      verticalOffset / INITIAL_SCALE - element.height / (2 * INITIAL_SCALE);
    ...
    const finalYOffset = yOffset - windowHeight / 2 + element.height;
  ...
  }, [centerZoom.current, INITIAL_SCALE, TRANSITION_TIME, Routes.timeline]);
